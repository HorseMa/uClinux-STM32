...
#define UART_CLOCK	(AT91X40_MASTER_CLOCK / 16)
...
static void serpoll(struct work_struct *work);
...
static _INLINE_ void transmit_chars(struct atmel_serial *info)
{
	...
	if (info->xmit_tail + info->xmit_cnt < SERIAL_XMIT_SIZE) {
		...
		info->xmit_cnt = info->xmit_cnt - (SERIAL_XMIT_SIZE - info->xmit_tail);
		info->xmit_tail = 0;
	}
 ...
 */
static irqreturn_t rs_interrupta(int irq, void *dev_id)
{
	return rs_interrupt(&atmel_info[0]);
}
static irqreturn_t rs_interruptb(int irq, void *dev_id)
{
...
#endif
	if (!info->use_ints)
		schedule_work(&info->intpoll);
	return IRQ_HANDLED;
}

static void serpoll(struct work_struct *work)
{
	struct atmel_serial *info = container_of(work, struct atmel_serial, intpoll);
...
{
	struct atmel_serial *info = container_of(work, struct atmel_serial, tqueue);
...
{
	struct atmel_serial *info = container_of(work, struct atmel_serial, tqueue_hangup);
	struct tty_struct *tty;
...
static void init_console(struct atmel_serial *info)
{
	...
	info->irq = AT91X40_ID_USART1;
#else
	info->usart = (volatile struct atmel_usart_regs *) AT91_USART0_BASE;
	info->irqmask = 1<<AT91X40_ID_USART0;
	info->irq = AT91X40_ID_USART0;
#endif
	info->tty = 0;
	info->port = 0;
	info->use_ints = 0;
...
{
	setup_irq(AT91X40_ID_USART0, &irq_usart0);
	setup_irq(AT91X40_ID_USART1, &irq_usart1);
}
...
rs_atmel_init(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		info->usart = usarts[i];
		info->tty = 0;
		info->irqmask = (i) ? (1<<AT91X40_ID_USART1) : (1<<AT91X40_ID_USART0);
		info->irq = (i) ? AT91X40_ID_USART1 : AT91X40_ID_USART0;
#ifdef CONFIG_SWAP_ATMEL_PORTS
		info->port = (i) ? 2 : 1;
		info->line = !i;
#ifdef CONFIG_SERIAL_AT91_ARM7_CONSOLE
		info->is_cons = i;
#else
		info->is_cons = 0;
#endif	
		...
		INIT_WORK(&info->intpoll, serpoll);
