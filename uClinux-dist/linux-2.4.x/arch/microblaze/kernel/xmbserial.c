...
/* Helper macro to save verbiage in the UART descriptor table */
#define UART_TABLE_ENTRY(id) \
{	magic : 0,						\
	addr  : CONFIG_XILINX_UARTLITE_##id##_BASEADDR,		\
	irq   : CONFIG_XILINX_UARTLITE_##id##_IRQ,		\
	flags : ASYNC_BOOT_AUTOCONF,				\
	baud  : CONFIG_XILINX_UARTLITE_##id##_BAUDRATE 		\
},
...
static struct xmb_serial xmbrs_table[] = {
		...
		UART_TABLE_ENTRY(3)
	#endif
	#ifdef CONFIG_XILINX_UARTLITE_4_INSTANCE
		UART_TABLE_ENTRY(4)
	#endif
/* Support the MDM UART, if configured - it will always be the last TTY */
	#if defined(CONFIG_XILINX_MDM_0_INSTANCE) && \
		CONFIG_XILINX_MDM_0_USE_UART && \
		CONFIG_XILINX_MDM_0_IRQ
  	{ 	magic : 0, 
		addr  : CONFIG_XILINX_MDM_0_BASEADDR,
		irq   : CONFIG_XILINX_MDM_0_IRQ, 
		flags : ASYNC_BOOT_AUTOCONF,
		baud  : 19200 /* Dummy baud rate */ }
	#endif /* CONFIG_XILINX_MDM_0_INSTANCE) */
};
