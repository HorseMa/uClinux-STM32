...
#define	CARDBUS_MEMORY_SIZE	0x00200000	/* 2MB regions */
#define	CARDBUS_IO_SIZE		0x00001000	/* 4k regions */

void pciauto_setup_cardbus(struct pci_controller *hose,
		int current_bus,
		int pci_devfn)
{
	/* Allocate the two Cardbus memory regions */
	pciauto_upper_memspc &= ~(CARDBUS_MEMORY_SIZE - 1);
	early_write_config_dword(hose,
			current_bus,
			pci_devfn,
	...
	pciauto_upper_memspc -= CARDBUS_MEMORY_SIZE;
	early_write_config_dword(hose,
			current_bus,
			pci_devfn,
	...
	/* Allocate the two Cardbus IO regions */
	pciauto_upper_iospc &= ~(CARDBUS_IO_SIZE - 1);
	early_write_config_dword(hose,
			current_bus,
			pci_devfn,
			PCI_CB_IO_LIMIT_0,
	...
	pciauto_upper_iospc -= CARDBUS_IO_SIZE;
	early_write_config_dword(hose,
			current_bus,
			pci_devfn,
			PCI_CB_IO_BASE_0,
	...
	for (pci_devfn=0; pci_devfn<0xff; pci_devfn++)
	{
		...
		if (vid != 0xffff)
		{
			...
			else
			{
				...
				/* Allocate PCI I/O and/or memory space */
				if ( (pci_class >> 16) == PCI_CLASS_BRIDGE_CARDBUS )
				{
					DBG("PCI Autoconfig: Found CardBus bridge\n");
#ifdef CONFIG_MACH_SG8100
					sg8100_cardbus_fixup(hose, current_bus, pci_devfn);
#endif
					pciauto_setup_bars(hose,
							current_bus,
							pci_devfn,
							PCI_BASE_ADDRESS_0);
					pciauto_setup_cardbus(hose,
						current_bus,
						pci_devfn);
				}
				else
