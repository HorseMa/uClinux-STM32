...
extern long C5471tags;

static void __init
fixup_c5471(struct machine_desc *desc, struct param_struct *params,
            char **cmdline, struct meminfo *mi)
{
	struct tag *t = (struct param_struct *)C5471tags;
  ...
  init_task_union.task.personality = PER_LINUX_32BIT;
	...
	for (; t->hdr.size; t = tag_next(t)) 
	{
		...
		{
			strncpy(*cmdline, t->u.cmdline.cmdline, COMMAND_LINE_SIZE);
			*cmdline[COMMAND_LINE_SIZE - 1] = '\0';
		}
	}
}

MACHINE_START(VC547X, "VC547X")
...
#if defined(CONFIG_MACH_UC5471DSP) || defined(CONFIG_MACH_SJ5471ENG)
#include <linux/sched.h>       /* For union task_union */
#include <linux/personality.h> /* For PER_LINUX_32BIT */
...
MACHINE_START(UC5471DSP, "uC-C5471DSP")
        MAINTAINER("Oleksandr ZHadan <oleks@ArcturusNetworks.com>")
        FIXUP(fixup_c5471)
        INITIRQ(genarch_init_irq)
MACHINE_END
#endif
#if defined(CONFIG_MACH_SJ5471ENG)
MACHINE_START(SJ5471ENG, "SJ5471-ENG")
        MAINTAINER("Oleksandr ZHadan <oleks@ArcturusNetworks.com>")
        FIXUP(fixup_c5471)
        INITIRQ(genarch_init_irq)
MACHINE_END
