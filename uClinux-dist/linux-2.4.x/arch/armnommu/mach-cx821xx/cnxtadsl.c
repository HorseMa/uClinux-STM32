	...
	switch (HWSTATE.eADSLLineState)
	{
		...
		case ADSL_SHOWTIME_UP:
			if (!InShowtime)
			{
				...
				// NOTE: The order of the calls to DmaCloseChannel for the
				// DSL channels must be in the exact reverse order as the
				// calls to DmaOpenChannel.  This is because of a currently
				// unsophisticated memory alloc/dealloc scheme in the 
				// sysmem.c source file.
