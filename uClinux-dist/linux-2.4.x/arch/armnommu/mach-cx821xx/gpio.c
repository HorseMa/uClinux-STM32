...
*
*	Copyright:		(c) 2002 - 2003  Conexant Systems Inc.
*
*****************************************************************************
...
#ifdef CONFIG_CNXT_WLAN
#include "../../../drivers/net/wireless/wlan.h"
#endif
...
GPIO_STATUS GPIO_Init(void)
{
   ...
   UINT32 uGPIODefaultValue[RUSHMORE_NUM_GPIODEFAULT] =
   {
      ...
      0x00000000, /*p_GRP0_ISM2: default to level IRQ*/
               ...
               0x00000040,	/* GPIO_OPT: Enable all GPIO except GPIO38.  HCS4 (GPIO37 CS for UART1), HCS2 (GPIO34 CS for 8975), HCS1 (GPIO33 CS for UART2) */
               0x0000ffbf,	/* p_GRP0_OUTPUT_ENABLE */  /* All output for testing except: GPIO14 (used for BOPT) */
               0x0000bdff,	/* p_GRP1_OUTPUT_ENABLE */  /* GPIO31 (8975 Reset). All output for testing except: GPIO25 (UART1 IRQ), GPIO30 (UART2 IRQ) */
               0x000000ff,	/* p_GRP2_OUTPUT_ENABLE */  /* All output for testing */
               0xffff3300,	/* p_GRP0_DATA_OUT */
               0xfffffff1,	/* p_GRP1_DATA_OUT */
               0xffff007f,	/* p_GRP2_DATA_OUT */
          #else
    ...
    // Change per APP. Note Rev. D
    cpu_type = WLANGetCpuType();
    if (cpu_type == CX82310_FAMILY)
    {
        *p_GRP2_OUTPUT_ENABLE |= BIT(1);    // GPIO33 for WLAN reset
        *p_GRP1_OUTPUT_ENABLE &= ~BIT(10);

        HWSTATE.p_GRP0_INPUT_ENABLE_Image |= BIT(14);
        *p_GRP1_INPUT_ENABLE = HWSTATE.p_GRP0_INPUT_ENABLE_Image;
        HWSTATE.p_GRP1_INPUT_ENABLE_Image |= (BIT(1) | BIT(9) | BIT(10));
        *p_GRP1_INPUT_ENABLE = HWSTATE.p_GRP1_INPUT_ENABLE_Image;

        wlan_reset_gpio = GPIO33;
    }
    else    // CX82110-51 or Cx82100-41
    {
        if (cpu_type == CX82110_51_FAMILY)
        {
            *GPIO_OPT |= BIT(4);                // GPIO36 set to GPIO
            *p_GRP2_OUTPUT_ENABLE |= BIT(4);    // GPIO36 for WLAN reset
            wlan_reset_gpio = GPIO36;
        }

        if (cpu_type == CX82100_41_FAMILY)
        {
            *p_GRP1_OUTPUT_ENABLE |= BIT(1);    // GPIO17 for WLAN reset
            wlan_reset_gpio = GPIO17;
        }
    }

    // Mask GPIO INT if CX-82110-51 or CX82100-41
    if (cpu_type == CX82110_51_FAMILY || cpu_type == CX82100_41_FAMILY)
    {
        *p_GRP1_OUTPUT_ENABLE &= ~BIT(5);       // GPIO21 for WLAN INT (input)
        *p_GRP1_INT_MASK &= ~BIT(21);           // Disable GPIO21 INT for WLAN
    }
    // Reset wireless card
    if (cpu_type == CX82310_FAMILY || cpu_type == CX82110_51_FAMILY || cpu_type == CX82100_41_FAMILY)
    {
        WriteGPIOData(wlan_reset_gpio,1);
        for(reset_delay=0;reset_delay<0x100000;reset_delay++);
        WriteGPIOData(wlan_reset_gpio,0);
        for(reset_delay=0;reset_delay<0x100000;reset_delay++);
    }
    #endif // CONFIG_CNXT_WLAN
