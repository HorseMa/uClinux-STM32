...
int get_irq_list(char *buf)
{
	...
	*p++ = '\n';
	...
	for (grp = 0; grp < NR_IRQ_GROUPS; grp++) {
		group = irq_groups[grp];
		if (!group)
			continue;

		for (ix = 0; ix < NR_IRQ_ACTIONS_PER_GROUP ; ix++) {
			...
			p += sprintf(p, "%3d: ", (grp << NR_IRQ_LOG2_ACTIONS_PER_GROUP) + ix);
#ifndef CONFIG_SMP
			p += sprintf(p, "%10u ", kstat_irqs(level));
#else
			for (j = 0; j < smp_num_cpus; j++)
				p += sprintf(p, "%10u ", kstat.irqs[cpu_logical_map(j)][level]);
#endif
			p += sprintf(p, " %12s@%x", group->sources[ix]->muxname, level);
	...
	p += sprintf(p, "LOC: ");
	for (j = 0; j < smp_num_cpus; j++)
		p += sprintf(p, "%10u ", apic_timer_irqs[cpu_logical_map(j)]);
	p += sprintf(p, "\n");
#endif
 ...
 * - we use the IRQ level as the IRQ number
