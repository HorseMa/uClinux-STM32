...
#if defined(CONFIG_MTD_NETtel) && !defined(CONFIG_COMMAND_LINE)
	/*
	 * Check for obvious recover boot args,  otherwise use the in flash
	 * bootargs and assume the recover options are bogus
	 */
	while (from < COMMAND_LINE + COMMAND_LINE_SIZE) {
		if (*from == 0xff || *from == '\0') {
			from = NULL;
			break;
		}
		if (!memcmp(from, "root=", 5))
			break;
		from++;
	}
	if (from == NULL) {
		from = (char *) 0x000e2000;
		if (*from == 0xff) /* check for erased args */
			from = "";
	} else
		from = COMMAND_LINE;
#endif
