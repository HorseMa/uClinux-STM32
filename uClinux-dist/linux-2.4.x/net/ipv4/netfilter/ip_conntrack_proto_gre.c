			  ...
			  struct sk_buff *skb, int hooknum)
{
	...
	if (ct->status & IPS_SEEN_REPLY) {
		ip_ct_refresh_acct(ct, ctinfo, skb->nh.iph,
				ct->proto.gre.stream_timeout);
		/* Also, more likely to be important, and not a probe. */
		set_bit(IPS_ASSURED_BIT, &ct->status);
	} else
		ip_ct_refresh_acct(ct, ctinfo, skb->nh.iph, ct->proto.gre.timeout);
	
	return NF_ACCEPT;
}

/* Called when a new connection for this protocol found. */
static int gre_new(struct ip_conntrack *ct, struct sk_buff *skb, int hooknum)
{ 
	DEBUGP(": ");
