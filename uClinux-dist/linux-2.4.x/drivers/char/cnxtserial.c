...
/*
 * Multiport capability added by Johann Hanne
 * Crash-bugs fixed by Stephen Clark
 */
...
static void wait_EOT(struct uart_regs *uart)
{
  ...
  while(1){
    if(timeout-- == 0) {
      printk(KERN_DEBUG "__FILE__:__LINE__:eot timeout\n");
      break;
    }
    if(puart->line_status_reg & LSR_Tx_Fifo_Empty) {
      break;
    } else {
      /* This is required for the Actiontec DPCM internal modem to work */
...
static void uart_speed(struct cnxt_serial *info)
{
  ...
  tx_start(uart, info->use_ints);
  start_rx(info);
  uart->modem_ctrl=15; /* Seems to be required for the Actiontec DPCM internal modem to work */
}
...
int rs_open(struct tty_struct *tty, struct file * filp)
{
...
#ifdef CONFIG_BD_TIBURON
	/* On the Actiontec DPCM ttyS1 is connected
	...
	if (retval) {
         ...
         * but the Actiontec DPCM internal modem did not work with it
         */
	if (info->count == 1) {
...
static inline void receive_chars(struct cnxt_serial *info, unsigned long status)
{
	...
	/* Weirdness: For some reason this must be *before* the
           "if (!(info->flags..." line, otherwise the Actiontec
           DPCM will sometimes reboot...?! */
	...
	if(info->is_cons)
	{
		...
		{ 
			HARD_RESET_NOW(); /* currently doesn't do anything sec
			                     06-28-2006 */
			return;
		}
	}
...
static void cnxtserial_interrupt(int line)
{
	struct cnxt_serial *info = &(uart_info[line]);
	struct uart_regs *uart = info->uart;
	char status;
...
{
   cnxtserial_interrupt(0);
   ClearGPIOIntStatus(GPIOINT_UART1);
}

#ifdef CONFIG_BD_TIBURON
/* Internal modem on the Actiontec DPCM */
void GPIOB5_Handler(int gpio_num)
{
   cnxtserial_interrupt(1);
...
rs_cnxt_init(void)
{
	...
	for (i = 0, info = uart_info; i < NR_PORTS; i++, info++) {
		...
		info->uart = (struct uart_regs*)(i == 0 ? UART0_Base_Addr : UART1_Base_Addr);
		info->port = (i == 0 ? UART0_Base_Addr : UART1_Base_Addr);
		info->tty = 0;
		info->irq = CNXT_INT_LVL_GPIO;
		...
		printk("%s%d at 0x%08x (irq = %d, gpio = %d)",
                       serial_driver.name, info->line, 
		       info->port, info->irq,
                       (i == 0 ? GPIOINT_UART1 : GPIOINT_UART2));
