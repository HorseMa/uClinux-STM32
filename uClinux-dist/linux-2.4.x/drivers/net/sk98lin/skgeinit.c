...
static const char SysKonnectFileId[] =
	"@(#) $Id: skgeinit.c,v 2.56 2004/08/13 09:38:27 rschmidt Exp $ (C) Marvell.";
#endif
...
SK_IOC	IoC)		/* I/O Context */
{
	...
	case CHIP_ID_YUKON_FE:
		pAC->GIni.HwF.Features[HW_DEV_LIST] = HWF_WA_DEV_427;
		break;
...
SK_IOC	IoC)		/* I/O Context */
{
	...
	if ((pAC->GIni.GIChipId >= CHIP_ID_YUKON_XL) &&
		...
		if ((DWord & PCI_OS_PCI_X) != 0) {
			...
			pAC->GIni.GIPciSlot64 = (SK_BOOL)((DWord & PCI_OS_PCI64B) != 0);
	...
	if (pAC->GIni.GIChipId != CHIP_ID_GENESIS) {
		...
		if (CHIP_ID_YUKON_2(pAC)) {
			/* for PCI-Express the PCI_CLK is substituted by the core clock */
			if (pAC->GIni.GIPciBus == SK_PEX_BUS) {
				...
				if (pAC->GIni.GIChipId == CHIP_ID_YUKON_FE) {
					...
					/* Yukon-EC/XL (PEX) work with 125 MHz host clock */
