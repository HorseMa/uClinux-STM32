...
* $Id: rtl_glue.c,v 1.1.2.1 2007/09/28 14:42:22 davidm Exp $
*/
...
/*  ==============================================
	Processor related processing
     ============================================== */
static inline int32 rtlglue_flushDCache(void);
#endif
...
static inline int32 rtlglue_flushDCache(void)
{
	...
	/*
		Processor : RLX5181
	*/
	dma_cache_wback_inv(0, r3k_cache_size(ST0_ISC));
...
void rtlglue_drvSend(void *data)
{
	...
	/* Due to RLX5181's write-back property, we ALWAYS flush D-CACHE before TXing */
	rtlglue_flushDCache();
