...
* Author :  
* $Id: rtl8651_tblDrvLocal.h,v 1.1.2.1 2007/09/28 14:42:31 davidm Exp $
...
{
	/*internal use*/
	uint32 age;	/*age for cache update time*/
	uint32 maxAge;	/*max age of the cache time*/
	uint32 flag;		/*flag*/
	uint32 action;	/*for ps...*/
	uint32 algindex; 	/*for alg*/
	uint32 valid;		/*the entry is valid?*/

	/*information for external using is defined in tbldrv.h*/
}rtl8651_tblDrv_naptUsrMapping_internal_t;

typedef struct _rtl8651_tblDrv_naptFlow_entry_s
{
	/*information for internal using*/
	rtl8651_tblDrv_naptUsrMapping_internal_t intInfo_naptUsrMappingEntry;
	...
	SLIST_ENTRY(_rtl8651_tblDrv_naptFlow_entry_s) nextNaptFlowEntry;	
}_rtl8651_tblDrv_naptFlow_entry_t;

/*remove from layer4local.h because we will merger with dmz/upnp/...*/
...
struct rtl8651_driverTables {
	...
	struct freeEntryList_s {
		...
		/*
		SLIST_HEAD(_FreeNaptUsrMappingEntry, _rtl8651_tblDrv_naptFlow_entry_s) naptUsrMappingEntry;
		*/
