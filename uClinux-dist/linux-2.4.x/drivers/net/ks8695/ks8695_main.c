...
/* these functions are not standard linux kernel funcitons. We added these in the consistent.c under /arch/mm */
/* it call virt_to_phys include virt_to_bus to get physical address */
extern void *consistent_alloc_ex(int gfp, size_t size, dma_addr_t *dma_handle);
extern void consistent_free_ex(void *vaddr, size_t size, dma_addr_t handle);
...
#ifdef	DEBUG_THIS
	DRV_INFO(" IO Address=0x%x", IO_ADDRESS(KS8695_IO_BASE));
#endif
...
int ks8695_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	Adapter->stDMAInfo.nBaseAddr = IO_ADDRESS(KS8695_IO_BASE);
	netdev->mem_start = IO_ADDRESS(KS8695_IO_BASE);
...
static void ks8695_fast_poll(void *arg)
{
	PADAPTER_STRUCT Adapter = ((struct net_device *)arg)->priv;
	int i, irq;
