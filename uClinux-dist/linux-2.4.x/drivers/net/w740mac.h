...
#define w740_WriteReg(reg,val,which)      (*((volatile unsigned int *)(_SYS_BASE_MAC+(which)*0x800+reg))=(val))
#define w740_ReadReg(reg,which)           (*((volatile unsigned int *)(_SYS_BASE_MAC+reg+(which)*0x800)))

#define w740_WriteCam1(which,x,lsw,msw) w740_WriteReg(CAM1L+(x)*CAM_ENTRY_SIZE,lsw,which);w740_WriteReg(CAM1M+(x)*CAM_ENTRY_SIZE,msw,which);
