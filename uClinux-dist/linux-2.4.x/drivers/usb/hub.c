...
static int usb_hub_configure(struct usb_hub *hub, struct usb_endpoint_descriptor *endpoint)
{
    ...
    /*
     * Caution: wHubCharacteristics is unaligned within struct usb_hub_descriptor,
     * so we must be careful to access it in such a way that the packed attribute
     * is retained.
     *
     * E.g. le16_to_cpus(&hub->descriptor->wHubCharacteristics) will result in
     * an unaligned access (and possibly data corruption) on some architectures,
     * because the packed attribute is not preserved when we take the address
     * and dereference.
     */
