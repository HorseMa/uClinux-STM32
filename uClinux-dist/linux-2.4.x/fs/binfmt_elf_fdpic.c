...
#ifdef NO_MM
static void elf_fdpic_transfer_args_to_stack(struct linux_binprm *bprm, unsigned long *_sp);
...
static int load_elf_fdpic_binary(struct linux_binprm *bprm, struct pt_regs *regs)
{
	...
	current->mm->exec_fdpic_loadmap = 0;
	current->mm->interp_fdpic_loadmap = 0;
	...
	current->mm->end_brk += (stack_size > PAGE_SIZE) ? (stack_size - PAGE_SIZE) : 0;
				   ...
				   struct elf_fdpic_params *interp_params)
{
	...
	elf_fdpic_transfer_args_to_stack(bprm, &sp);
#endif
	...
	copy_to_user((void *) sp, exec_params->loadmap, len);

	current->mm->exec_fdpic_loadmap = (unsigned long) sp;
	...
	if (interp_params->loadmap) {
		...
		copy_to_user((void *) sp, interp_params->loadmap, len);

		current->mm->interp_fdpic_loadmap = (unsigned long) sp;
	}
...
static void elf_fdpic_transfer_args_to_stack(struct linux_binprm *bprm, unsigned long *_sp)
{
	...
	for (index = MAX_ARG_PAGES - 1; index >= stop; index--) {
		...
		sp -= PAGE_SIZE;
		copy_to_user((void *) sp, src, PAGE_SIZE);
