...
volatile unsigned long *pending = (unsigned long*) AIC_IPR;
	...
	offset = (tc->cv * tick)/LATCH;

	// check if a timer interrupt is pending in the first half of the upcount
	if ((offset < tick/2) && (*pending & (1<<KERNEL_TIMER_IRQ_NUM)))
		offset+=tick;
	return offset;
}
...
extern inline void setup_timer (void)
{
	...
	v |= TCNXCNS(KERNEL_TIMER,1);
        ...
        tc->rc = LATCH;   /* load the count limit into the CR register */
