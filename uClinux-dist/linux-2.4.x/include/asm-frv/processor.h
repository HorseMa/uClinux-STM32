...
extern asmlinkage int arch_kernel_thread(int (*fn)(void *), void * arg, unsigned long flags);
...
static inline unsigned long thread_saved_pc(struct thread_struct *t)
{
	...
	if (t->pc > (unsigned long) scheduling_functions_start_here &&
	    t->pc < (unsigned long) scheduling_functions_end_here)
		return ((unsigned long *)t->fp)[2];
	else
		return t->pc;
}
...
extern void get_task_struct(struct task_struct *p);
