...
/*
 * Unaligned accesses on uClinux can't be performed in a fault handler
 * - the CPU detects them as imprecise exceptions making this
 * impossible.
 *
 * With the FR451, however, they are precise, and so we used to fix
 * them up in the memory access fault handler.  However, instruction
 * bundling make this impractical.  So, now we fall back to using
 * memcpy.
 */
...
extern int handle_misalignment(unsigned long esr0, unsigned long ear0,
			       unsigned long epcr0);
