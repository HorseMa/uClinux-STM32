...
{
	return (dma_addr_t) page_to_bus(page) + offset;
}
...
pci_dac_dma_sync_single(struct pci_dev *pdev, dma_addr_t dma_addr, size_t len, int direction)
{
	/* Nothing to do. */
}
...
/* Tell drivers/pci/proc.c that Microblaze does not have pci_mmap_page_range() */
#undef HAVE_PCI_MMAP
