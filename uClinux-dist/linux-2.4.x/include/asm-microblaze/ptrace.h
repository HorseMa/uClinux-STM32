...
struct pt_regs
{
	...
	microblaze_reg_t ear;		/* exception address register */
	microblaze_reg_t esr;		/* exception status register */
	microblaze_reg_t fsr;		/* FPU status register */

	microblaze_reg_t kernel_mode;	/* 1 if `kernel mode', 0 if user mode */
...
#define NUM_SPECIAL	7
#define PT_PC		((NUM_GPRS + 0) * _PT_REG_SIZE)
#define PT_PSW		((NUM_GPRS + 1) * _PT_REG_SIZE)
#define PT_EAR		((NUM_GPRS + 2) * _PT_REG_SIZE)
#define PT_ESR		((NUM_GPRS + 3) * _PT_REG_SIZE)
#define PT_FSR		((NUM_GPRS + 4) * _PT_REG_SIZE)
#define PT_KERNEL_MODE	((NUM_GPRS + 5) * _PT_REG_SIZE)
#define PT_SINGLESTEP	((NUM_GPRS + 6) * _PT_REG_SIZE)
