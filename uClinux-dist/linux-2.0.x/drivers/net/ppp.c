...
/* $Id: ppp.c,v 1.9 2001-10-02 07:15:46 gerg Exp $ */
...
ppp_first_time(void)
{
	...
	mppe_init();//Added By Soth to start mppe
	//Comment out this line to stop mppe encryption
			...
			if (skb->len >= ppp->mru + PPP_HDRLEN + 2
				...
				if (ppp->flags & SC_DEBUG)
					printk(KERN_DEBUG "rcv frame too long: "
					       ...
					       (int)skb->len, ppp->mru, skb_headroom(skb),
					       skb_tailroom(skb));
			} else {
...
ppp_ioctl(struct ppp *ppp, unsigned int param2, unsigned long param3)
{
	...
	case PPPIOCSMRU:
		...
		if (temp_i < PPP_MRU)
			temp_i = PPP_MRU;
		ppp->mru = temp_i;
		/* If we be encrypting, we'll adjust the mru up by the header
		 * size for mppe.
		 */
		save_flags(flags);
		cli();
		if (ppp->sc_xc_state && ppp->sc_xcomp->compress_proto == CI_MPPE) {
			ppp->mru += MPPE_OVHD;
			ppp->flags |= SC_COMP_MPPE;
		} else
			ppp->flags &= ~SC_COMP_MPPE;
		restore_flags(flags);
		if (ppp->flags & SC_DEBUG)
...
ppp_set_compression (struct ppp *ppp, struct ppp_option_data *odp)
{
	...
	/* Correct for any previous mru increase due to mppe */
	if (ppp->flags & SC_COMP_MPPE)
		ppp->mru -= MPPE_OVHD;
		...
		if (PPP_PROTOCOL(data) == PPP_COMP) {
			...
			if (new_count > 0) {
				...
				data = skb_put(skb, count);
                                /* Most compression engines remove the PPP
                                   address compress the protocol word,
                                   Microsoft does not compress the
                                   protocol word, so to interoperate
                                   with Microsoft and those implementations
                                   that conserve octets, do the PPP frame
                                   expansion here */
                                if((PPP_ADDRESS(data) != PPP_ALLSTATIONS) ||
                                   (PPP_CONTROL(data) != PPP_UI)) {
                                    if(*data & 1) {
                                            data = skb_push(skb,1);
                                            *data = 0;
...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (((ppp->flags & SC_COMP_RUN) != 0)	&&
		...
		/* Figure out how much space we've got to allocate MPPE requires
		 * extra for its overhead.  This does mean we can exceed mtu by
		 * the overhead bytes (4) but it avoids a nasty failure condition.
		 */
		buffer_size = ppp->mtu + PPP_HDRLEN;
		if (ppp->sc_xcomp->compress_proto == CI_MPPE)
			buffer_size += MPPE_OVHD;
		
		/* Allocate an skb for the compressed frame. */
		new_skb = alloc_skb(buffer_size, GFP_ATOMIC);
		if (new_skb == NULL) {
